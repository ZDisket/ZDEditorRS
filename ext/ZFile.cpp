...
void ZFile::operator>>(GString &Gsout)
{
    size_t szout = 0;
    Read(szout);
    Gsout.resize(szout);
    Read((void*)Gsout.data(),szout * sizeof(wchar_t));


}

void ZFile::operator<<(const GString &Gsin)
{
    Write(Gsin.size());
    Write((void*)Gsin.data(),Gsin.size() * sizeof(wchar_t));
