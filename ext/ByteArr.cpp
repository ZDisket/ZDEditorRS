...
void ByteArr::operator<<(const GString & inGS)
{
    // Add the size
    (*this) << inGS.size();

    Add((void*)inGS.data(), inGS.size() * sizeof(wchar_t));

}

void ByteArr::operator>>(GString & outGS)
{
    ...
    Read((void*)outGS.data(), rsz * sizeof(wchar_t));
